...
**pnpm workspaces:** `apps/*`, `packages/*`. **Turbo:** `turbo.json` for `build` / `dev` / `lint` / `typecheck`.
...
2. Extend `packages/typescript-config` if needed; wire eslint from `@better-starter/eslint-config`.
