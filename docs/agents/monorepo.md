...
**Lists (server pages):** URL state via app `list/` (`useList`, `ListSearch`, **`ListFooter`**, params helpers) — do **not** put search/filter/pagination in the grid, and do **not** use ReUI `DataGridPagination` for these lists. Render with ReUI `DataGrid` + `DataGridTable` inline (copy `members-table.tsx`). Filter chips: inline shadcn `ToggleGroup`. Columns: `createDataGridColumnHelper`. Detail: [dashboard.md § Server lists](./dashboard.md#server-lists).
